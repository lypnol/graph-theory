...
def opt_two_all(matrix, time_limit = 1.8):
    ...
    while improved:
        ...
        while perms and not improved:
            ...
            sol = list(solution)
            temp_partial_sol = list(reversed(sol[min(elt1, elt2):max(elt1, elt2)+1]))
            for i in range(min(elt1, elt2), max(elt1, elt2) + 1):
                sol[i] = temp_partial_sol[i - (min(elt1, elt2))]
